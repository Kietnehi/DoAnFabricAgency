...
$staff_stmt = $conn->prepare("
    ...
    FROM employee e
    JOIN supplier s ON e.ECode = s.SCode
    WHERE e.Role = 'PartnerStaff' AND e.SCode = ?
");
...
<body>
    ...
    <?php if (count($partner_staff) > 0): ?>
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td><?= htmlspecialchars($staff['ECode']); ?></td>
    ...
    <?php else: ?>
        <p class="no-data">Không có nhân viên đối tác nào liên quan đến nhà cung cấp này.</p>
    <?php endif; ?>
